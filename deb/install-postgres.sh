...
# deb/install-postgres.sh - Instalação do PostgreSQL (Debian/Ubuntu)
...
if [ "${PG_INSTALLED}" = "false" ]; then
  ...
  apt-get update -qy
  if ! apt-get install -y "postgresql-${POSTGRES_VERSION}" "postgresql-contrib-${POSTGRES_VERSION}"; then
    msg_error "Falha na instalação do PostgreSQL ${POSTGRES_VERSION}."
    exit 1
  fi

  # --- 7. Iniciar e habilitar serviço ---
  # No Debian, o apt já inicializa o cluster e inicia o serviço
...
if ! grep -q "${_db_user}" "${PG_HBA}" 2>/dev/null; then
  ...
  # Adicionar antes da primeira regra existente
...
echo "  Porta:     5432"
echo "  Datadir:   /var/lib/postgresql/${POSTGRES_VERSION}/main"
echo "  pg_hba:    ${PG_HBA}"
echo ""
