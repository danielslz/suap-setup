...
# arch/install-postgres.sh - Instalação do PostgreSQL (Arch Linux)
# Instala o PostgreSQL via pacman, inicializa o cluster,
...
# Versão do PostgreSQL (informativa no Arch — pacman instala a versão do repositório)
: "${POSTGRES_VERSION:=16}"
...
if [ "${PG_INSTALLED}" = "false" ]; then
  # --- 5. Instalar pacote ---
  msg_action "Instalando PostgreSQL..."
  if ! pacman -S --needed --noconfirm postgresql; then
    msg_error "Falha na instalação do PostgreSQL."
    exit 1
  fi
  ...
  if [ ! -f "${PG_DATADIR}/PG_VERSION" ]; then
    ...
    sudo -u postgres initdb -D "${PG_DATADIR}" --locale pt_BR.UTF-8 --encoding UTF8
  fi

  # --- 7. Iniciar e habilitar serviço ---
  systemctl enable postgresql
  systemctl start postgresql
  msg_action "PostgreSQL instalado e em execução."
fi
...
if [[ ! "${_criar_banco}" =~ ^[sS]$ ]]; then
  ...
  echo ""
  echo "  Versão:  PostgreSQL ($(psql --version | grep -oP '[\d.]+'))"
  echo "  Serviço: systemctl status postgresql"
  echo "  Porta:   5432"
  echo ""
  exit 0
fi
...
echo ""
echo "  Versão:    $(psql --version | grep -oP '[\d.]+')"
echo "  Banco:     ${_db_name}"
echo "  Usuário:   ${_db_user}"
echo "  Porta:     5432"
echo "  Datadir:   /var/lib/postgres/data"
echo "  pg_hba:    ${PG_HBA}"
echo ""
