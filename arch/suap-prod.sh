...
# arch/suap-prod.sh - Configuração do ambiente de produção SUAP (Arch Linux)
...
BASE="base-devel vim git openssl supervisor curl postgresql-libs tmpwatch swig cronie chrony"
PYTHON="python python-virtualenv python-pip"
LDAP="libldap libsasl"
PILLOW="libjpeg-turbo freetype2 zlib"
PYMSSQL="freetds"
...
if ! check_all_packages_installed $ALL_PACKAGES; then
    ...
    if ! pacman -S --needed --noconfirm $ALL_PACKAGES; then
...
if [ "$CURRENT_LANG" != "pt_BR.UTF-8" ]; then
    ...
    # Garantir que o locale está gerado
...
# --- Configurar Supervisor (Arch-specific: supervisord) ---
...
if [ "$FILES_COPIED" = "true" ]; then
    # Ajustar usuário nos .ini do Supervisor (www-data → http para Arch)
    sed -i 's/www-data/http/g' "$SUPERVISOR_CONF_DIR"/*.ini 2>/dev/null || true
